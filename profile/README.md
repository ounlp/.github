<!--
  This file is the OUNLP org homepage (renders at https://github.com/ounlp).
  BANNER: drop a wide image at profile/assets/banner.png (≈1280×320) and
  uncomment the block below for a stronger first impression.
-->
<!--
<p align="center">
  <a href="https://ounlp.org">
    <img src="profile/assets/banner.png" alt="OUNLP — OU Natural Language Processing Lab" width="100%">
  </a>
</p>
-->

<h1 align="center">
  <img src="profile/assets/logo.png" alt="OUNLP logo" height="36" valign="middle">
  OUNLP — OU Natural Language Processing Lab
</h1>

<p align="center">
  <em>Through Wind and Rain.</em> Building robust, interpretable AI systems that support
  <b>human learning, communication, and collaboration.</b>
</p>

<p align="center">
  School of Computer Science · University of Oklahoma · Norman, OK<br>
  Directed by <a href="https://ounlp.org/team/">Dr. Jie Cao</a>, Assistant Professor
</p>

<p align="center">
  <a href="https://ounlp.org">🌐 Website</a> ·
  <a href="https://ounlp.org/research/">🔬 Research</a> ·
  <a href="https://ounlp.org/research/">📚 Publications</a> ·
  <a href="https://ounlp.org/team/">👥 Team</a> ·
  <a href="https://ounlp.org/news/">📰 News</a> ·
  <a href="mailto:jie.cao@ou.edu">✉️ Contact</a>
</p>

---

### 🔬 What we study

We work toward **reliable human–AI language systems** — from multi-party dialogue and LLM
agents to structured prediction and trustworthy deployment in education, healthcare, and science.

| Theme | |
|---|---|
| **🗣️ Dialogue & Discourse** | Multi-party, multi-modal dialogue systems and discourse-aware modeling. |
| **🧩 Structured Prediction** | Discourse-aware representation learning and structured prediction over language. |
| **🤖 LLM Alignment & Agents** | LLM alignment and agentic systems that act and collaborate. |
| **🎛️ Multimodal Interaction** | Grounded communication across speech, vision, and text. |
| **🔭 AI for Science** | ML for discovery across materials, chemistry, and the physical sciences. |
| **🛡️ Trustworthy AI** | Reliable, interpretable AI for education and healthcare. |

<p align="center"><a href="https://ounlp.org/research/"><b>Browse publications by theme →</b></a></p>

### ⭐ Highlighted work

- **A Multimodal Large Language Model for Materials Science** — *Nature Machine Intelligence* (2026) · [doi](https://doi.org/10.1038/s42256-026-01214-y)
- **HydroAgent: Closing the Gap Between Frontier LLMs and Human Experts in Hydrologic Model Calibration via Simulator-Grounded RL** — *arXiv* (2026) · [arXiv:2605.17792](https://arxiv.org/abs/2605.17792)

### 🛠️ Open source

| Project | What it is |
|---|---|
| [Multi-Round-Text-Simplifier](https://github.com/ounlp/Multi-Round-Text-Simplifier) | Iterative, multi-round text simplification |
| [dialogue-act](https://github.com/ounlp/dialogue-act) | Dialogue-act modeling |
| [lab-website](https://github.com/ounlp/lab-website) | Source of [ounlp.org](https://ounlp.org) |

### 📰 Latest from the lab
<!-- Auto-updated from ounlp.org/feed.xml by .github/workflows/update-news.yml -->
<!-- BLOG-POST-LIST:START -->- [AgentBeats SDK, AgentX–AgentBeats Competition, and OUNLP Project](https://ounlp.org/2025/11/12/agent-beats.html) <!-- Wed Nov 12 2025 12:00 AM -->
<!-- BLOG-POST-LIST:END -->

### 🎓 Join us

We welcome students and collaborators excited about NLP/ML for dialogue, agents, and
trustworthy AI. See [**ounlp.org**](https://ounlp.org) or email
[jie.cao@ou.edu](mailto:jie.cao@ou.edu).

<p align="center"><sub>🌪️ Inspired by Oklahoma's powerful skies — staying focused through uncertainty, and advancing discovery.</sub></p>
